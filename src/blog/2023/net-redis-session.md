...
pubDate: 2023-03-08
description: 'Session 是Web開發中可能會碰到的東西﹐雖然使用Session對於應用系統來說壞處多於益處﹐但有時在某些場景下或許有不得不用的情形﹐先不談其它的﹐Session就像是個全域性變數﹐隨時都可宣告﹐隨地都可修改內容﹐又任何時候都可信手拈來的使用﹐非常方便﹐可是就因為太方便了﹐若沒有好好的控管﹐很容易讓系統在出了問題時很難追查原因。在我曾接手過的案子就常見這種讓人恨的牙癢癢的系統。雖然要避免濫用Session﹐但還是不得不要了解Session的使用﹐由其是在分散式環境下要如何解決Session共用的問題。'
author: '州. Kevin'
image:
    url: ''
    alt: ''
...
![net-redis-session-01](images/net-redis-session-01.jpg)

這裏為了儘量和實際上線後的環境相同﹐建立兩個虛擬主機 A 和 B﹐並且都安裝了Docker以容器化來執行﹐主機 A 中安裝了Nginx container 做為 loadbalancer﹐另外將應.Net Core 應用程式建立 Image 後產生三個不同port的container 來模擬不同的AP Server﹐主機 B 則安裝了Redis container供主機A中的應用程式共享Session﹐Redis for Docker 如何安裝參考 Redis Server Install.docx文件中的描述。(做完後我才想起來﹐我用 docker 幹嘛這麼搞剛的多弄了一台虛擬主機呢…=\_\_=!)
...
![net-redis-session-02](images/net-redis-session-02.jpg)
...
![net-redis-session-03](images/net-redis-session-03.jpg)
...
![net-redis-session-04](images/net-redis-session-04.jpg)![net-redis-session-05](images/net-redis-session-05.jpg)

5. 建立 Container

$ docker run -it -d -p 80:80 --name=dist dist:1.0

![net-redis-session-06](images/net-redis-session-06.jpg)
...
![net-redis-session-10](images/net-redis-session-10.jpg)
...
![net-redis-session-11](images/net-redis-session-11.jpg)
...
![net-redis-session-12](images/net-redis-session-12.jpg)

4. 執行程式測試

在執行程式前我們先觀察一下目前三個應用程式的container id﹐注意目前程式只有使用一般的session﹐還未做任何分散式的做法

![net-redis-session-13](images/net-redis-session-13.jpg)
...
![net-redis-session-14](images/net-redis-session-14.jpg)

按下登入後並沒有進入系統而是又回到了登入頁﹐這時看到畫面的Host Name變成了**445991c3dbc5**

![net-redis-session-15](images/net-redis-session-15.jpg)
...
![net-redis-session-16](images/net-redis-session-16.jpg)
...
![net-redis-session-17](images/net-redis-session-17.jpg)

分別是9acb188f1c6f, 505ed16ba9c5, d05be6917a64﹐現在重新執行程式

![net-redis-session-18](images/net-redis-session-18.jpg)

一開始的Host Name是d05be6917a64, 按下登入後畫面依舊是回到了登入頁﹐Host Name變為9acb188f1c6f

![net-redis-session-19](images/net-redis-session-19.jpg)

這和我們預期的不一樣﹐我們看一下Another Redis Desktop Manager

![net-redis-session-20](images/net-redis-session-20.jpg)

畫面上Redis確實有資料﹐在剛剛程式中Program.cs中加入的代碼﹐給的InstanceName是”.dist.Session”

![net-redis-session-21](images/net-redis-session-21.jpg)
...
![net-redis-session-22](images/net-redis-session-22.jpg)

但程式並沒有登入畫面﹐這是為什麼？如果再去看看應用程式container的log會發現和先前的錯誤並沒有什麼兩樣﹐不過其實在之前的Log已經有線索了。

![net-redis-session-23](images/net-redis-session-23.jpg)
...
![net-redis-session-24](images/net-redis-session-24.jpg)

現在新的3個container id為**aac2aa57a55a, 9af461700e14, 79aebc644f00**﹐然後再執行程式測試

![net-redis-session-25](images/net-redis-session-25.jpg)

現在Host Name為9af461700e14﹐按下登入後這次順利登入了﹐在Home的頁面也帶出了Login時所寫入的Session的值﹐而畫面上這時出現的Host Name為79aebc644f00和登入頁是不同的

![net-redis-session-26](images/net-redis-session-26.jpg)

這時如果去點擊Page2﹐可以看到Host Name又不一樣了

![net-redis-session-27](images/net-redis-session-27.jpg)
...
[ASP.NET Core 中Session的分布式存储\_FatTigerWang的博客-CSDN博客\_core 存储session](https://blog.csdn.net/FatTigerWang/article/details/120162415)

[Nginx Proxy建立Load Balance分流機制 – Paul's Recipe Book (g) (paulfun.net)](http://paulfun.net/wordpress/?p=645)

[集群环境下，你不得不注意的ASP.NET Core Data Protection 机制\_dotNET跨平台的博客-CSDN博客](https://blog.csdn.net/sD7O95O/article/details/102546783)
