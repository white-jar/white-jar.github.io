...
從 2009 年開始在 [點部落 DotBlogs](https://www.dotblogs.com.tw/nethawk) 利用工作之餘偶爾寫寫一些筆記, 但平時研究技術﹑工作繁忙加上不夠積極, 到 2024 年也只寫了 51 篇, 真是少啊。 最近又想寫些心得, 但這幾年因為 AI 的緣故習慣寫 markdown 文件, 所以想重新找個免費﹑支援 md 又不要太複雜的 blog 框架。參考了許多的介紹後決定採用 GitHub Pages 並加上 Astro 來搭建自己的 blog。 這篇文章主要是將整個過程做個記錄, 避免日後要重建或者遇到問題時能參考。
...
## 2. 建立一個 Repository
登入 github 後 create new repository, 要當做 blog 這個 **repository** 的名稱必須是 **<使用者帳號>.github.io**, 另外, 要當 blog 必須這個 repository 要設定為 `public`。
...
![seggings-pages](images/2026-1_3.png)
...
在本機開啟 powershell 或命例列模式(我個人喜歡用 Cmder), 切換到想要建立之後這個 blog 編輯的位置, 以 `create astro` 安裝創建一個新的 Astro 項目, 注意, 執行後會詢問專案名稱並<span style="background-color:yellow">建立專案資料夾</span>, 所以<span style="background-color:yellow">代表不能先將 GitHub 的 Repository 同步下來</span>, 為了後續要和 GitHub repository 名稱一致, 我會命名為相同的名稱
...
```
詢問專案名稱, 這裏填上專案名稱, 會建立一個以此專案名稱的資料夾
![create astro](images/2026-1_11.png)

詢問這個專案的用途, 會先給一個 template(有興趣的人可以每一個試試看)  
![create astor](images/2026-1_12.png)

詢問要不要安裝依賴項目, 當然是安裝, 免得之後的麻煩
![create astro](images/2026-1_13.png)

詢問要不要初始化 git repository, 這裏回答**Yes**
...
在本機首先確認在專案目錄之下, 在前面 create astro 過程中有一個選項詢問是否要初始化 git repository (Initialize a new respostiory?) 
...
# 解決衝突
# 因為之前建立遠端 GitHub repository 同時建立了 README.md, 而本地端 create astro 也會產生一個 README.md, 上一步的強制合併, 會導致衝突, 所以打開 README.md 檔案內容會有衝突的區段
...
要解決這個錯誤, 現在需要開啟 `.github\worlflows\astro.yml` 做修改
找到以下的節段
...
## 後記
Astro 是一個靜態網頁生成器, 可以將 Markdown 轉換成靜態網頁, 除了一開始建置專案時可選擇的樣式之外, Astro 官網也有提供其它主題 [Astro 主題](https://astro.build/themes) , 在網路上也有許多人建置了其它主題供人套用, 如果不想花時間和精神可以考慮看看, 不過既然自已都寫 code 那麼多年了, 加上現在 AI 的便利, 之前寫過 Vue 然後看看 Astro 官網的教學後, 用 AI 就可以根據自己的想法建了自己喜歡的主題, 現在這個頁面就是自行結合 web-design-engineer skill 及 openspce 框架 vibe coding 出來的, 有幾個區塊功能重複, 因為還還沒想好要放什麼, 就先用 tag 頂著, 畫面比較好看些。
